                ...
                if (err) {
                    ...
                    try {
                        ...
                        var spawn = require('child_process').spawn,
                            rubySass = spawn('sass'),
                            css = '';
                        rubySass.stderr.setEncoding('utf8');
                        rubySass.stderr.on('data', function(data) {
                            require('sys').log(data);
                            css += data;
                        });
                        rubySass.stdout.setEncoding('utf8');
                        rubySass.stdout.on('data', function(data) {
                            css += data;
                        });
                        rubySass.on('exit', function(code) {
                            if (code === 0)
                                fn(null, css);
                            else
                                fn('sass exited with code: ' + code);
                        });
                        rubySass.stdin.write(new Buffer(str));
                        rubySass.stdin.end();
                    } catch (err) {
                        fn(err);
                    }
